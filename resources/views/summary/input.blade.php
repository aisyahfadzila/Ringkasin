<x-app-layout>
    <x-slot name="title">| Tulis Ringkasan</x-slot>
    <section class="max-w-screen-xl mx-auto p-4">
        <a href="{{ route('summary.content', ['book_id' => $summary_id, 'summary_id' => $book_id]) }} "
            class="my-2 px-2 hover:bg-gray-100 hover:rounded-md">&#60; Kembali ke memilih buku</a>
        <form method="POST" action="{{ route('summary.input', ['summary_id' => $summary_id, 'book_id' => $book_id]) }}"
            ...
            <div class="border-2 rounded-lg flex flex-col w-full mx-4 p-4">
                ...
                <textarea name="content" id="" class="border-none min-h-screen indent-defaultx"></textarea>
            </div>
